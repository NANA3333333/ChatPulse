...
记忆库不再展示横向时间线/可视化表，也不应请求或返回 `timeline` 数据。来源场景与时间标签仍可作为维护补充流程的一部分保留，但不能再驱动记忆库页面的可视化时间线。

- 旧库备份只作为迁移来源和备份查看，不参与 RAG 和统计。
- 新版记忆库页面只保留统计、角色筛选、遗忘曲线、分类列表和维护工具。
- 不再新增 `timeline_filter`、`timeline_all`、`timeline_limit` 前端入口或后端 library 响应字段。
