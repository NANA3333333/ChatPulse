...
- 对话会进入上下文、缓存、摘要、长期记忆、朋友圈、日记、商业街事件等后续系统。
...
3. 用户消息写入 `messages`。
4. engine 构造上下文，包括角色人设、用户资料、最近历史、摘要、RAG 召回、朋友圈、日记、城市细节等。
...
8. 回复解析结构化标签，例如好感、朋友圈、日记、情绪、商业街意图、转账反应等。
...
记忆不只来自私聊。群聊、朋友圈、日记、商业街日志、外部聊天记录都可能进入记忆系统。
...
朋友圈是角色和用户的公开动态层，支持图片、点赞、评论和可见性。日记是角色私密表达层，使用角色密码解锁。

这两个模块的价值不只是 UI 展示：

- 朋友圈可以成为后续对话和关系判断的上下文。
- 日记可以作为角色隐藏状态和情绪连续性的输入。
- 删除或清空角色数据时，朋友圈和日记也需要随角色一并处理。

### 4.6 群聊

群聊由 `server/plugins/groupChat` 提供。它支持创建群、编辑群、增删成员、群消息、AI 回复、批量删除、暂停 AI、无链模式和主动消息。
...
主题系统保存于用户资料中，支持 CSS 变量、自定义 CSS、AI 主题生成和导入导出。

前端通过全局 CSS 变量影响侧边栏、联系人列表、聊天区域、气泡、输入框等区域。AI 生成主题时必须返回可解析的主题配置，不能直接执行不可信脚本。

### 4.15 备份与恢复

备份插件提供系统导出、导入和擦除。角色包导入导出则在主路由中处理，覆盖聊天、记忆、朋友圈、日记以及可重建 Qdrant 索引的数据。
...
- 社交内容：`moments`、`moment_likes`、`moment_comments`、`diaries`。
- 用户资料：`user_profile`。
...
- `/api/user`、`/api/user/memory-status`：用户资料、主题、钱包、记忆后端状态。
...
- `/api/moments*`：朋友圈列表、发布、删除、点赞、评论。
- `/api/diaries*`：日记读取、解锁、删除。
- `/api/groups*`：群聊、成员、消息、AI 暂停、无链模式、红包。
